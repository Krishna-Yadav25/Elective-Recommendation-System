        ...
        <div class="section-card">
            ...
            <div id="list" class="section">
                ...
                <% List<Document> electives = (List<Document>) request.getAttribute("electives");
                   if (electives != null && !electives.isEmpty()) { for (Document e : electives) { String diff = e.getString("difficulty") != null ? e.getString("difficulty") : "Easy"; String pc = "Easy".equals(diff) ? "pill-easy" : "Medium".equals(diff) ? "pill-medium" : "pill-hard"; %>
            ...
            <div id="feedbacks" class="section">
                ...
                <div class="section-body">
                <%try{com.mongodb.client.MongoClient fbClient=com.mongodb.client.MongoClients.create("mongodb://localhost:27017");com.mongodb.client.MongoCollection<Document> fbCol=fbClient.getDatabase("electiveDB").getCollection("feedback");List<Document> allFeedback=fbCol.find().into(new java.util.ArrayList<>());
                if(allFeedback.isEmpty()){%><div style="text-align:center;padding:36px 0"><div style="font-size:14px;font-weight:700;color:#1a1a2e;margin-bottom:5px">No feedback received yet</div><div style="font-size:12px;color:#9ca3af">Students will appear here once they submit reviews.</div></div>
            ...
            <!-- ═══════════════════════════════════════
                 ANNOUNCEMENTS — ADMIN POST & MANAGE
                 ═══════════════════════════════════════ -->
            <div id="announce" class="section">
                <div class="section-head">
                    <div>
                        <div class="section-title">Announcements</div>
                        <div class="section-sub">Post notices to all students — they see them on their dashboard instantly</div>
                    </div>
                    <span class="badge badge-green">Broadcasts to all students</span>
                </div>
                <div class="section-body">
                    ...
                    <div style="background:#f5f3ff;border:1px solid #AFA9EC;border-radius:12px;padding:20px;margin-bottom:22px">
                        ...
                        <form action="announcement" method="post" id="annForm">
                            <input type="hidden" name="type" id="typeInput" value="info">
                            <div class="form-grid" style="margin-bottom:14px">
                                <div class="field" style="grid-column:span 2">
                                    <label>Title</label>
                                    <input type="text" name="title" placeholder="e.g. Elective enrollment opens Monday">
                                </div>
                                <div class="field" style="grid-column:span 2">
                                    <label>Message</label>
                                    <textarea name="message" rows="3" placeholder="Write your announcement here... Students will see this on their dashboard."></textarea>
                                </div>
                                <div class="field" style="grid-column:span 2">
                                    <label>Type</label>
                                    <div class="type-grid">
                                        <div class="type-btn sel-info" id="type-info" onclick="setType('info')">
                                            ℹ Info
                                        </div>
                                        <div class="type-btn" id="type-warning" onclick="setType('warning')">
                                            ⚠ Warning
                                        </div>
                                        <div class="type-btn" id="type-success" onclick="setType('success')">
                                            ✓ Success
                                        </div>
                                    </div>
                                </div>
                    ...
                    <%
                    try {
                        com.mongodb.client.MongoClient annClient = com.mongodb.client.MongoClients.create("mongodb://localhost:27017");
                        com.mongodb.client.MongoCollection<Document> annCol =
                            annClient.getDatabase("electiveDB").getCollection("announcements");
                        List<Document> annList = annCol.find()
                        ...
                        <% } else {
                            for (Document ann : annList) {
                                String atype = ann.getString("type") != null ? ann.getString("type") : "info";
                                String annId = ann.getObjectId("_id").toHexString();
                                boolean isActive = Boolean.TRUE.equals(ann.getBoolean("active"));
                        %>
                        <div class="ann-card <%= atype %>" id="ann-<%= annId %>">
                            <div class="ann-card-icon">
                                <% if ("warning".equals(atype)) { %>
                                <svg viewBox="0 0 16 16" fill="none"><path d="M8 3L14 13H2L8 3z" stroke="#CA8A04" stroke-width="1.3" fill="none" stroke-linejoin="round"/><path d="M8 7v3M8 11.5v.5" stroke="#CA8A04" stroke-width="1.3" stroke-linecap="round"/></svg>
                                <% } else if ("success".equals(atype)) { %>
                                <svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5.5" stroke="#1D9E75" stroke-width="1.3"/><path d="M5.5 8l2 2 3-3" stroke="#1D9E75" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                <% } else { %>
                                <svg viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="#534AB7" stroke-width="1.3" fill="none"/><path d="M5 7h6M5 10h4" stroke="#534AB7" stroke-width="1.2" stroke-linecap="round"/></svg>
                                <% } %>
                            </div>
                            <div class="ann-card-body">
                                <div class="ann-card-title"><%= ann.getString("title") %></div>
                                <div class="ann-card-msg"><%= ann.getString("message") %></div>
                                <div class="ann-card-meta">
                                    Posted by <%= ann.getString("postedBy") %>
                                    &nbsp;·&nbsp; <%= ann.getString("timestamp") != null ? ann.getString("timestamp").substring(0,24) : "" %>
                                    &nbsp;·&nbsp;
                                    <span style="color:<%= isActive ? "#1D9E75" : "#9ca3af" %>; font-weight:600">
                                        <%= isActive ? "Active" : "Hidden" %>
                                    </span>
                                </div>
                            </div>
                            <div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">
                                <button class="btn-sm-red" onclick="deleteAnn('<%= annId %>')">Delete</button>
                            </div>
                        </div>
                        <%  }
                        }
                        ...
                        out.println("<p style='color:#E24B4A;font-size:13px;'>Error loading announcements.</p>");
                    } %>
                </div>
            </div>
            <!-- ═══════════ END ANNOUNCEMENTS ═══════════ -->
...
/* Auto-open section from URL param */
(function () {
    const sec = '<%= openSec %>';
...
function setType(type) {
    ...
    ['info','warning','success'].forEach(t => {
        const btn = document.getElementById('type-' + t);
        btn.className = 'type-btn' + (t === type ? ' sel-' + t : '');
    });
}

/* Delete announcement via fetch */
function deleteAnn(id) {
    if (!confirm('Delete this announcement? Students will no longer see it.')) return;
    fetch('announcement?id=' + id, { method: 'DELETE' })
        .then(r => {
            if (r.ok) {
