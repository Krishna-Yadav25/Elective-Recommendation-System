...
<%
    ...
    try {
        ...
        com.mongodb.client.MongoCollection<Document> nCol = nClient.getDatabase("electiveDB").getCollection("notifications");
        notifList = nCol.find(eq("studentId", studentId)).sort(new Document("_id",-1)).limit(20).into(new ArrayList<>());
        for (Document nd : notifList) if (!Boolean.TRUE.equals(nd.getBoolean("read"))) unreadCount++;
        nClient.close();
    } catch (Exception ex) {}
    ...
    try {
        ...
        com.mongodb.client.MongoCollection<Document> aCol = aClient.getDatabase("electiveDB").getCollection("announcements");
        announcements = aCol.find(eq("active", true)).sort(new Document("_id",-1)).limit(10).into(new ArrayList<>());
        aClient.close();
    } catch (Exception ex) {}
%>
<!DOCTYPE html>
...
.notif-item{padding:12px 16px;border-bottom:1px solid #f5f5fa;display:flex;gap:10px;align-items:flex-start;transition:background .1s}
...
/* ══ ELECTIVE CARD v2 (with syllabus) ══ */
.elective-card-v2{border:1px solid #e8e8f0;border-radius:12px;margin-bottom:12px;background:#fff;overflow:hidden;transition:box-shadow .15s,border-color .15s}
.elective-card-v2:hover{box-shadow:0 4px 20px rgba(83,74,183,.08);border-color:#AFA9EC}
.elective-card-v2-head{display:flex;align-items:center;gap:14px;padding:14px 16px;cursor:pointer;user-select:none}
.elective-num{width:28px;height:28px;border-radius:8px;background:#EEEDFE;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#534AB7;flex-shrink:0}
.elective-info{flex:1}
.elective-name{font-size:14px;font-weight:700;color:#1a1a2e}
.elective-meta{font-size:12px;color:#6b7280;margin-top:3px;display:flex;align-items:center;gap:7px;flex-wrap:wrap}
.elective-badge{font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px}
.eb-ai{background:#EEEDFE;color:#3C3489}
.eb-web{background:#E1F5EE;color:#085041}
.eb-cyber{background:#FAEEDA;color:#633806}
.eb-data{background:#E6F1FB;color:#0C447C}
.eb-general{background:#f1f0ef;color:#57534e}
.eb-easy{background:#E1F5EE;color:#085041}
.eb-medium{background:#FAEEDA;color:#633806}
.eb-hard{background:#FEE2E2;color:#991B1B}
.elective-chevron{color:#9ca3af;transition:transform .2s;flex-shrink:0}
.elective-chevron.open{transform:rotate(180deg)}
.elective-body{display:none;border-top:1px solid #f0f0f8}
.elective-body.open{display:block}
.elective-desc{padding:12px 16px;font-size:13px;color:#6b7280;line-height:1.65;border-bottom:1px solid #f0f0f8}
/* PDF viewer */
.syllabus-area{padding:14px 16px}
.syllabus-bar{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:12px}
.syllabus-label{font-size:12px;font-weight:700;color:#1a1a2e;display:flex;align-items:center;gap:7px}
.syllabus-label svg{width:14px;height:14px;color:#534AB7}
.syllabus-actions{display:flex;gap:7px;flex-wrap:wrap}
.btn-view-pdf{padding:7px 14px;background:#534AB7;color:#fff;border:none;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;text-decoration:none}
.btn-view-pdf:hover{background:#3C3489}
.btn-view-pdf svg{width:12px;height:12px}
.btn-dl-pdf{padding:7px 14px;background:#f0f2ff;color:#534AB7;border:1px solid #AFA9EC;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:5px;text-decoration:none}
.btn-dl-pdf:hover{background:#EEEDFE}
.btn-dl-pdf svg{width:12px;height:12px}
.pdf-iframe-wrap{border:1px solid #e0e0f0;border-radius:10px;overflow:hidden;background:#fafafa;display:none}
.pdf-iframe-wrap.show{display:block}
.pdf-iframe-wrap iframe{width:100%;height:500px;border:none;display:block}
.no-syllabus{display:flex;align-items:center;gap:8px;padding:12px 14px;background:#fafafa;border:1px dashed #e0e0f0;border-radius:9px;font-size:12px;color:#9ca3af}
.no-syllabus svg{width:14px;height:14px;flex-shrink:0}
.syl-avail-tag{display:inline-flex;align-items:center;gap:3px;font-size:11px;color:#1D9E75;font-weight:600}
.syl-avail-tag svg{width:11px;height:11px}
</style>
</head>
<body>
...
<div class="topbar">
    ...
    <div class="topbar-right">
        <div class="user-chip"><div class="av"><%= initials %></div>Welcome, <%= name %></div>
        <!-- BELL -->
        <div class="bell-wrap" id="bellWrap">
            <div class="bell-btn" onclick="toggleNotifDropdown()">
                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M8 2a4 4 0 0 1 4 4v2l1 2H3l1-2V6a4 4 0 0 1 4-4z"/><path d="M6.5 12a1.5 1.5 0 0 0 3 0" stroke-linecap="round"/></svg>
            </div>
            <div class="notif-badge <%= unreadCount > 0 ? "show" : "" %>" id="notifBadge"><%= unreadCount > 9 ? "9+" : String.valueOf(unreadCount) %></div>
            <div class="notif-dropdown" id="notifDropdown">
                <div class="notif-drop-head">
                    <span class="notif-drop-title">Notifications</span>
                    <% if (unreadCount > 0) { %><button class="mark-all-btn" onclick="markAllRead()">Mark all as read</button><% } %>
                </div>
                <div class="notif-list">
                    <% if (notifList.isEmpty()) { %><div class="notif-empty">No notifications yet</div>
                    <% } else { for (Document nd : notifList) { boolean isUnread = !Boolean.TRUE.equals(nd.getBoolean("read")); String ntype = nd.getString("type") != null ? nd.getString("type") : "info"; String iconBg = "announcement".equals(ntype) ? "#FEF3C7" : "query".equals(ntype) ? "#E1F5EE" : "#EEEDFE"; String iconColor = "announcement".equals(ntype) ? "#B45309" : "query".equals(ntype) ? "#0F6E56" : "#534AB7"; %>
                    <div class="notif-item <%= isUnread ? "unread" : "" %>">
                        ...
                        <div class="notif-text"><div class="notif-msg"><%= nd.getString("message") %></div><div class="notif-time"><%= nd.getString("timestamp") != null ? nd.getString("timestamp").substring(0,24) : "" %></div></div>
                        <% if (isUnread) { %><div class="unread-dot"></div><% } %>
                    </div>
                    <% } } %>
                </div>
                <div class="notif-drop-foot"><a onclick="showSection('notifications',document.getElementById('nav-notifications'));closeNotifDropdown()">View all notifications</a></div>
            </div>
        </div>
...
<div class="layout">
    ...
    <div class="sidebar">
        ...
        <div class="nav-item" id="nav-academic" onclick="showSection('academic',this)">
            <svg viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="5" height="5" rx="1" fill="#534AB7"/><rect x="9" y="2" width="5" height="5" rx="1" fill="#534AB7" opacity=".4"/><rect x="2" y="9" width="5" height="5" rx="1" fill="#534AB7" opacity=".4"/><rect x="9" y="9" width="5" height="5" rx="1" fill="#534AB7" opacity=".4"/></svg>Academic
        </div>
        <div class="nav-item" id="nav-recommend" onclick="showSection('recommend',this)">
            <svg viewBox="0 0 16 16" fill="none"><path d="M8 2L9.8 6.2H14L10.6 8.8 11.8 13 8 10.4 4.2 13 5.4 8.8 2 6.2H6.2Z" stroke="currentColor" stroke-width="1.2" fill="none" stroke-linejoin="round"/></svg>Recommend
        </div>
        ...
        <div class="nav-item" id="nav-query" onclick="showSection('query',this)">
            <svg viewBox="0 0 16 16" fill="none"><path d="M3 3h10a1 1 0 0 1 1 1v6a1 1 0 0 1-1 1H5l-3 2V4a1 1 0 0 1 1-1z" stroke="currentColor" stroke-width="1.2" fill="none"/></svg>Query
        </div>
        <div class="nav-item" id="nav-answers" onclick="showSection('answers',this)">
            <svg viewBox="0 0 16 16" fill="none"><path d="M3 4h10M3 8h7M3 12h5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>My Queries
        </div>
        <div class="nav-item" id="nav-electives" onclick="showSection('electives',this)">
            <svg viewBox="0 0 16 16" fill="none"><rect x="2" y="4" width="12" height="9" rx="1.5" stroke="currentColor" stroke-width="1.2" fill="none"/><path d="M5 4V3a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v1" stroke="currentColor" stroke-width="1.2"/></svg>Electives
        </div>
        <div class="nav-item" id="nav-selected" onclick="showSection('selected',this)">
            <svg viewBox="0 0 16 16" fill="none"><path d="M2 5h12M2 8h8M2 11h5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><circle cx="13" cy="11" r="2.5" stroke="currentColor" stroke-width="1.2"/><path d="M12.3 11l.7.7 1.3-1.3" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/></svg>My Electives
        </div>
        ...
        <div class="nav-item" id="nav-announcements" onclick="showSection('announcements',this)">
            ...
            Announcements
            <% if (!announcements.isEmpty()) { %><span style="margin-left:auto;background:#534AB7;color:#fff;font-size:10px;font-weight:700;padding:1px 6px;border-radius:10px;"><%= announcements.size() %></span><% } %>
        </div>
        ...
        <div class="nav-item" onclick="openSwing()">
            <svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="5.5" r="2.5" stroke="currentColor" stroke-width="1.2"/><path d="M3 14c0-2.76 2.24-5 5-5s5 2.24 5 5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>Student Profile
        </div>
    </div>
    ...
    <div class="content">
        ...
        <div class="ann-feed">
            <% for (Document ann : announcements) { String atype = ann.getString("type") != null ? ann.getString("type") : "info"; %>
            <div class="ann-banner <%= atype %>">
                ...
                <div><div class="ann-title"><%= ann.getString("title") %></div><div class="ann-msg"><%= ann.getString("message") %></div><div class="ann-meta">Posted by Admin &nbsp;·&nbsp; <%= ann.getString("timestamp") != null ? ann.getString("timestamp").substring(0,24) : "" %></div></div>
            </div>
            <% } %>
        </div>
        <% } %>
        ...
        <div class="hero">
            ...
            <div class="hero-inner">
                <div class="big-av"><% if (photo != null && !photo.isEmpty()) { %><img src="images/<%= photo %>" alt="Photo"><% } else { %><%= initials %><% } %></div>
                <div class="hero-info">
                ...
                <div class="hero-upload">
                    <form action="uploadPhoto" method="post" enctype="multipart/form-data" style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
        ...
        <!-- STAT CARDS -->
        <div class="stats-row">
            <div class="sc"><div class="sc-top" style="background:#534AB7"></div><div class="sc-icon" style="background:#EEEDFE"><svg width="14" height="14" viewBox="0 0 16 16"><path d="M8 2L10 6h4l-3 3 1 4-4-2-4 2 1-4-3-3h4z" fill="#534AB7"/></svg></div><div class="sc-label">CGPA</div><div class="sc-value" id="statCgpa"><%= statCgpa %></div><div class="sc-sub">out of 10.0</div></div>
            <div class="sc"><div class="sc-top" style="background:#1D9E75"></div><div class="sc-icon" style="background:#E1F5EE"><svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M2 12L5 8L8 10L12 5" stroke="#1D9E75" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="sc-label">10th Score</div><div class="sc-value" id="statTenth"><%= statTenth %></div><div class="sc-sub">board exam</div></div>
            <div class="sc"><div class="sc-top" style="background:#BA7517"></div><div class="sc-icon" style="background:#FAEEDA"><svg width="14" height="14" viewBox="0 0 16 16" fill="none"><rect x="3" y="7" width="3" height="6" rx="1" fill="#BA7517" opacity=".5"/><rect x="7" y="4" width="3" height="9" rx="1" fill="#BA7517" opacity=".75"/><rect x="11" y="2" width="3" height="11" rx="1" fill="#BA7517"/></svg></div><div class="sc-label">Coding Level</div><div class="sc-value" id="statCoding" style="font-size:16px;margin-top:7px"><%= statCoding %></div><div class="sc-sub">self-reported</div></div>
            <div class="sc"><div class="sc-top" style="background:#185FA5"></div><div class="sc-icon" style="background:#E6F1FB"><svg width="14" height="14" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5" stroke="#185FA5" stroke-width="1.5"/><path d="M8 5v3l2 2" stroke="#185FA5" stroke-width="1.5" stroke-linecap="round"/></svg></div><div class="sc-label">Goal</div><div class="sc-value" id="statGoal" style="font-size:16px;margin-top:7px"><%= statGoal %></div><div class="sc-sub">career track</div></div>
        </div>
        ...
        <div class="section-card">
            ...
            <div id="answers" class="section">
                ...
                <div class="section-body">
                <%try{com.mongodb.client.MongoClient mclient=com.mongodb.client.MongoClients.create("mongodb://localhost:27017");com.mongodb.client.MongoCollection<Document> mcol=mclient.getDatabase("electiveDB").getCollection("queries");Document q=mcol.find(eq("studentId",studentId)).first();if(q!=null){List<Document> messages=(List<Document>)q.get("messages");%>
                    ...
                    <%if(messages!=null){for(Document m:messages){String sender=m.getString("sender");String text=m.getString("text");boolean isStu="student".equals(sender);%>
                        <div class="msg <%= isStu?"student":"admin" %>"><div class="bubble"><div class="sender"><%= sender %></div><%= text %><% if("admin".equals(sender)){%><form action="satisfy" method="post" class="satisfy-btns"><button name="status" value="satisfied" class="btn-sat yes">Satisfied</button><button name="status" value="not_satisfied" class="btn-sat no">Not satisfied</button></form><%}%></div></div>
                    <%}}%></div>
                ...
                <%}else{%><p style="color:#9ca3af;font-size:13px;margin-bottom:14px;">No queries yet.</p><form action="query" method="post" class="chat-input"><input type="text" name="query" placeholder="Ask something..."><button type="submit" class="btn-send">Send</button></form><%}mclient.close();}catch(Exception ex){out.println("<p style='color:#E24B4A;font-size:13px;'>Error loading queries.</p>");}%>
                </div>
            </div>

            <!-- ═══ ELECTIVES WITH SYLLABUS ═══ -->
            <div id="electives" class="section">
                <div class="section-head">
                    <div><div class="section-title">Available Electives</div><div class="section-sub">Click any elective to expand — view or download syllabus PDF</div></div>
                ...
                try {
                    ...
                    if (elist.isEmpty()) { %>
                        <div style="text-align:center;padding:36px 0">
                            <div style="width:52px;height:52px;background:#EEEDFE;border-radius:14px;display:flex;align-items:center;justify-content:center;margin:0 auto 12px"><svg width="24" height="24" viewBox="0 0 16 16" fill="none"><rect x="2" y="4" width="12" height="9" rx="1.5" stroke="#534AB7" stroke-width="1.3"/><path d="M5 4V3a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v1" stroke="#534AB7" stroke-width="1.3"/></svg></div>
                            <div style="font-size:14px;font-weight:700;color:#1a1a2e;margin-bottom:5px">No electives found</div>
                            <div style="font-size:12px;color:#9ca3af">Admin hasn't added any electives yet.</div>
                        </div>
                    <% } else { int idx=1; for (Document e : elist) {
                        String eName  = e.getString("name")        != null ? e.getString("name")        : "Unnamed";
                        String eDom   = e.getString("domain")      != null ? e.getString("domain")      : (e.getString("category") != null ? e.getString("category") : "General");
                        String eDiff  = e.getString("difficulty")  != null ? e.getString("difficulty")  : "—";
                        String eDesc  = e.getString("description") != null ? e.getString("description") : "";
                        String eSyl   = e.getString("syllabusFile");
                        String domCls = "eb-general";
                        String eDomL  = eDom.toLowerCase();
                        if (eDomL.contains("ai")||eDomL.contains("machine")) domCls="eb-ai";
                        else if (eDomL.contains("web")||eDomL.contains("dev")) domCls="eb-web";
                        else if (eDomL.contains("cyber")||eDomL.contains("security")) domCls="eb-cyber";
                        else if (eDomL.contains("data")) domCls="eb-data";
                        String diffCls = "Medium".equalsIgnoreCase(eDiff)?"eb-medium":"Hard".equalsIgnoreCase(eDiff)?"eb-hard":"eb-easy";
                        String safeId  = "elec-"+idx;
                        String encName = java.net.URLEncoder.encode(eName,"UTF-8");
                    %>
                    <div class="elective-card-v2" id="<%= safeId %>-card">
                        <div class="elective-card-v2-head" onclick="toggleElective('<%= safeId %>')">
                            <div class="elective-num"><%= idx %></div>
                            <div class="elective-info">
                                <div class="elective-name"><%= eName %></div>
                                <div class="elective-meta">
                                    <span class="elective-badge <%= domCls %>"><%= eDom %></span>
                                    <span class="elective-badge <%= diffCls %>"><%= eDiff %></span>
                                    <% if (eSyl != null && !eSyl.isEmpty()) { %>
                                    <span class="syl-avail-tag"><svg viewBox="0 0 16 16" fill="none"><rect x="3" y="2" width="10" height="12" rx="1.5" stroke="currentColor" stroke-width="1.3"/><path d="M5 6h6M5 9h6M5 12h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>Syllabus available</span>
                                    <% } %>
                                </div>
                            </div>
                            <svg class="elective-chevron" id="<%= safeId %>-chev" width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                        </div>
                        <div class="elective-body" id="<%= safeId %>-body">
                            <% if (!eDesc.isEmpty()) { %><div class="elective-desc"><%= eDesc %></div><% } %>
                            <div class="syllabus-area">
                                <div class="syllabus-bar">
                                    <div class="syllabus-label">
                                        <svg viewBox="0 0 16 16" fill="none"><rect x="3" y="2" width="10" height="12" rx="1.5" stroke="currentColor" stroke-width="1.3"/><path d="M5 6h6M5 9h6M5 12h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>
                                        Syllabus PDF
                                    </div>
                                    <% if (eSyl != null && !eSyl.isEmpty()) { %>
                                    <div class="syllabus-actions">
                                        <button class="btn-view-pdf" onclick="togglePdf('<%= safeId %>','<%= encName %>')">
                                            <svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5.5" stroke="white" stroke-width="1.3"/><circle cx="8" cy="8" r="2" fill="white"/></svg>
                                            <span id="<%= safeId %>-vbtn">View PDF</span>
                                        </button>
                                        <a class="btn-dl-pdf" href="syllabus?electiveName=<%= encName %>" download>
                                            <svg viewBox="0 0 16 16" fill="none"><path d="M8 3v7M5 7l3 3 3-3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/><path d="M3 12h10" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>
                                            Download
                                        </a>
                                    </div>
                                    <% } %>
                                ...
                                <% if (eSyl != null && !eSyl.isEmpty()) { %>
                                <div class="pdf-iframe-wrap" id="<%= safeId %>-pdfwrap">
                                    <iframe id="<%= safeId %>-iframe" src="" title="Syllabus of <%= eName %>">
                                        <p>Your browser does not support PDFs. <a href="syllabus?electiveName=<%= encName %>" target="_blank">Download here</a></p>
                                    </iframe>
                                </div>
                                <% } else { %>
                                <div class="no-syllabus">
                                    <svg viewBox="0 0 16 16" fill="none"><rect x="3" y="2" width="10" height="12" rx="1.5" stroke="currentColor" stroke-width="1.3"/><path d="M6 7h4M6 10h2" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>
                                    Syllabus not uploaded yet — check back later.
                                </div>
                                <% } %>
                            </div>
                        </div>
            ...
            <div id="selected" class="section">
                ...
                <div class="section-body">
                <%try{com.mongodb.client.MongoClient sclient=com.mongodb.client.MongoClients.create("mongodb://localhost:27017");com.mongodb.client.MongoCollection<Document> scol=sclient.getDatabase("electiveDB").getCollection("selected_electives");List<Document> slist=scol.find(new Document("studentId",studentId)).into(new ArrayList<>());if(slist.isEmpty()){%><p style="color:#9ca3af;font-size:13px;">No electives selected yet.</p><%}else{for(Document e:slist){%><div class="elective-card green"><h3><%= e.getString("name") %></h3><p><%= e.getString("domain") %></p></div><%}}sclient.close();}catch(Exception ex){out.println("<p style='color:#E24B4A;font-size:13px;'>Error loading selected electives.</p>");}%>
                </div>
            </div>

            <!-- FEEDBACK -->
            <div id="feedback" class="section">
                <div class="section-head"><div><div class="section-title">Rate Your Electives</div><div class="section-sub">Share your experience — helps others choose better</div></div><span class="badge badge-amber">Visible to Admin</span></div>
                <div class="section-body">
                <%List<String> enrolledNames=new ArrayList<>();try{com.mongodb.client.MongoClient fbSelClient=com.mongodb.client.MongoClients.create("mongodb://localhost:27017");com.mongodb.client.MongoCollection<Document> fbSelCol=fbSelClient.getDatabase("electiveDB").getCollection("selected_electives");List<Document> fbSelList=fbSelCol.find(new Document("studentId",studentId)).into(new ArrayList<>());for(Document fe:fbSelList){String eName=fe.getString("name");if(eName!=null)enrolledNames.add(eName);}fbSelClient.close();}catch(Exception ex){}%>
                ...
                <%}else{%>
                    ...
                    <%try{com.mongodb.client.MongoClient pfClient=com.mongodb.client.MongoClients.create("mongodb://localhost:27017");com.mongodb.client.MongoCollection<Document> pfCol=pfClient.getDatabase("electiveDB").getCollection("feedback");List<Document> myFbs=pfCol.find(new Document("studentId",studentId)).into(new ArrayList<>());if(!myFbs.isEmpty()){%><div class="past-label">Your Previous Reviews</div><%for(Document fb:myFbs){int stars=fb.getInteger("rating",0);%><div class="fb-card"><div class="fb-card-head"><span class="fb-card-name"><%= fb.getString("electiveName") %></span><span class="fb-stars"><%for(int s=1;s<=5;s++){%><%= s<=stars?"★":"☆" %><%}%></span></div><%String cmt=fb.getString("comment");if(cmt!=null&&!cmt.isEmpty()){%><div class="fb-comment"><%= cmt %></div><%}%><div class="fb-date"><%= fb.getString("timestamp") %></div></div><%}}pfClient.close();}catch(Exception ex){}%>
                <%}%>
                </div>
            </div>
            ...
            <div id="announcements" class="section">
                <div class="section-head"><div><div class="section-title">Announcements</div><div class="section-sub">Important notices from admin</div></div><span class="badge badge-green"><%= announcements.size() %> Active</span></div>
                <div class="section-body">
                <%if(announcements.isEmpty()){%>
                    <div style="text-align:center;padding:36px 0"><div style="width:52px;height:52px;background:#EEEDFE;border-radius:14px;display:flex;align-items:center;justify-content:center;margin:0 auto 12px"><svg width="24" height="24" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="#534AB7" stroke-width="1.3" fill="none"/><path d="M5 7h6M5 10h4" stroke="#534AB7" stroke-width="1.2" stroke-linecap="round"/></svg></div><div style="font-size:14px;font-weight:700;color:#1a1a2e;margin-bottom:5px">No announcements yet</div><div style="font-size:12px;color:#9ca3af">Admin hasn't posted anything yet.</div></div>
                <%}else{for(Document ann:announcements){String atype=ann.getString("type")!=null?ann.getString("type"):"info";%>
                    <div class="ann-banner <%= atype %>" style="margin-bottom:10px">
                        <div class="ann-icon"><%if("warning".equals(atype)){%><svg viewBox="0 0 16 16" fill="none"><path d="M8 3L14 13H2L8 3z" stroke="currentColor" stroke-width="1.3" fill="none" stroke-linejoin="round"/><path d="M8 7v3M8 11.5v.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg><%}else if("success".equals(atype)){%><svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="5.5" stroke="currentColor" stroke-width="1.3"/><path d="M5.5 8l2 2 3-3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg><%}else{%><svg viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="currentColor" stroke-width="1.3" fill="none"/><path d="M5 7h6M5 10h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg><%}%></div>
                        <div><div class="ann-title"><%= ann.getString("title") %></div><div class="ann-msg"><%= ann.getString("message") %></div><div class="ann-meta">Posted by <%= ann.getString("postedBy") %> &nbsp;·&nbsp; <%= ann.getString("timestamp")!=null?ann.getString("timestamp").substring(0,24):"" %></div></div>
                    </div>
                <%}}%>
                </div>
            </div>
            ...
            <div id="notifications" class="section">
                <div class="section-head"><div><div class="section-title">All Notifications</div><div class="section-sub">Query replies, announcements, enrollments</div></div>
                <%if(unreadCount>0){%><button class="btn-primary" style="padding:7px 16px;font-size:12px" onclick="markAllRead()">Mark all read (<%= unreadCount %>)</button><%}else{%><span class="badge badge-green">All caught up</span><%}%></div>
                <div class="section-body">
                <%if(notifList.isEmpty()){%>
                    <div style="text-align:center;padding:36px 0"><div style="width:52px;height:52px;background:#EEEDFE;border-radius:14px;display:flex;align-items:center;justify-content:center;margin:0 auto 12px"><svg width="24" height="24" viewBox="0 0 16 16" fill="none"><path d="M8 2a4 4 0 0 1 4 4v2l1 2H3l1-2V6a4 4 0 0 1 4-4z" stroke="#534AB7" stroke-width="1.3" fill="none"/><path d="M6.5 12a1.5 1.5 0 0 0 3 0" stroke="#534AB7" stroke-width="1.3"/></svg></div><div style="font-size:14px;font-weight:700;color:#1a1a2e;margin-bottom:5px">No notifications yet</div><div style="font-size:12px;color:#9ca3af">You'll see query replies and announcements here.</div></div>
                <%}else{%>
                    <div class="notif-section-list">
                    <%for(Document nd:notifList){boolean isUnread=!Boolean.TRUE.equals(nd.getBoolean("read"));String ntype=nd.getString("type")!=null?nd.getString("type"):"info";String iconBg="announcement".equals(ntype)?"#FEF3C7":"query".equals(ntype)?"#E1F5EE":"#EEEDFE";String iconColor="announcement".equals(ntype)?"#B45309":"query".equals(ntype)?"#0F6E56":"#534AB7";%>
                    <div class="notif-row <%= isUnread?"unread-row":"" %>">
                        <div class="notif-row-icon" style="background:<%= iconBg %>"><%if("announcement".equals(ntype)){%><svg viewBox="0 0 16 16" fill="none" stroke="<%= iconColor %>" stroke-width="1.4"><rect x="2" y="3" width="12" height="10" rx="1.5" fill="none"/><path d="M5 7h6M5 10h4" stroke-linecap="round"/></svg><%}else if("query".equals(ntype)){%><svg viewBox="0 0 16 16" fill="none" stroke="<%= iconColor %>" stroke-width="1.4"><path d="M3 3h10a1 1 0 0 1 1 1v6a1 1 0 0 1-1 1H5l-3 2V4a1 1 0 0 1 1-1z" fill="none"/></svg><%}else{%><svg viewBox="0 0 16 16" fill="none" stroke="<%= iconColor %>" stroke-width="1.4"><circle cx="8" cy="8" r="5.5"/><path d="M8 5v3l2 2" stroke-linecap="round"/></svg><%}%></div>
                        <div class="notif-row-text"><div class="notif-row-msg"><%= nd.getString("message") %></div><div class="notif-row-time"><%= nd.getString("timestamp")!=null?nd.getString("timestamp").substring(0,24):"" %></div></div>
...
function markAllRead() {
    fetch('notification?action=markRead', { method: 'POST' }).then(() => {
        document.querySelectorAll('.unread-dot,.unread-pill').forEach(el => el.remove());
        document.querySelectorAll('.notif-item.unread,.notif-row.unread-row').forEach(el => el.classList.remove('unread','unread-row'));
        const badge = document.getElementById('notifBadge'); if (badge) { badge.classList.remove('show'); badge.textContent='0'; }
        const navDot = document.getElementById('navDot'); if (navDot) navDot.classList.remove('show');
        const markBtn = document.querySelector('.mark-all-btn'); if (markBtn) markBtn.remove();
    });
}

/* ── ELECTIVE EXPAND/COLLAPSE ── */
function toggleElective(id) {
    const body = document.getElementById(id + '-body');
    const chev = document.getElementById(id + '-chev');
    const isOpen = body.classList.contains('open');
    document.querySelectorAll('.elective-body').forEach(b => b.classList.remove('open'));
    document.querySelectorAll('.elective-chevron').forEach(c => c.classList.remove('open'));
    if (!isOpen) { body.classList.add('open'); chev.classList.add('open'); }
}

/* ── INLINE PDF TOGGLE ── */
function togglePdf(id, electiveName) {
    const wrap   = document.getElementById(id + '-pdfwrap');
    const iframe = document.getElementById(id + '-iframe');
    const btn    = document.getElementById(id + '-vbtn');
    const isShown = wrap.classList.contains('show');
    if (isShown) {
        wrap.classList.remove('show'); iframe.src = ''; btn.textContent = 'View PDF';
    } else {
        iframe.src = 'syllabus?electiveName=' + electiveName;
        wrap.classList.add('show'); btn.textContent = 'Hide PDF';
    }
}
...
(function () {
    ...
    stars.forEach(s => {
        s.addEventListener('click', function () {
            const val = +this.dataset.val; input.value = val;
        ...
        s.addEventListener('mouseenter', function () { const val = +this.dataset.val; stars.forEach(st => st.style.color = +st.dataset.val <= val ? '#f59e0b' : ''); });
...
document.querySelectorAll('.interest-btn').forEach(btn => {
    btn.addEventListener('click', function () { document.querySelectorAll('.interest-btn').forEach(b => b.classList.remove('selected')); this.classList.add('selected'); });
});
